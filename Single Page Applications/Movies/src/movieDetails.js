...
function changeMovieLikes(movieId) {
  ...
  .then(data => document.querySelector("#number-of-likes") ? 
                document.querySelector("#number-of-likes").textContent = `Likes: ${data}` 
                : null)
...
function removeLike(movieId) {
  ...
  fetch(`http://localhost:3030/data/likes/${movieId}`, {
    method: "DELETE",
    headers: {
